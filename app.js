...
async function fetchProduct(url) {
    ...
    const result = await page.evaluate(() => {
        ...
        for (let i = 1; i < totalSearchResults - 1; i++) {
            ...
            let rawPrice = document.querySelector(`#priceblock_ourprice`);
            product.price = rawPrice ? rawPrice.innerText : '';

            let rawRating = document.querySelector(`#acrPopover`);
            product.rating = rawRating ? rawRating.innerText : '';

            let rawFeedback = document.querySelector(`#acrCustomerReviewText`);
            product.Feedback = rawFeedback ? rawFeedback.innerText : '';

            if (typeof product.product !== 'undefined') {
                !product.product.trim() ? null : productsList = product.concat(product);
            }
        }
